...
def search_contactByName(nm, tx1, tx2, tx3, tx4, tx5, tx6, vo):
    ...
    for i in find:
        ...
        if button is None:
            button = tk.Button(
                vo,
                text="Agregar set de Datos",
                command=lambda: add_set(vo),
                name="add_set_button",
            )
            button.pack()
...
def list_all(tx1, tx2, tx3, tx4, tx5, tx6, vo):
    ...
    for i, document in enumerate(find):
        ...
        labels.append((new_label1, new_label2))

        datos_contacto = document['datos_de_contacto']
        for contacto in datos_contacto:
            new_label3 = tk.StringVar()
            new_label4 = tk.StringVar()
            new_label5 = tk.StringVar()

            new_label3.set(f"Categoria: {contacto['categoria']}")
            new_label4.set(f"Direccion: {contacto['direccion']}")
            new_label5.set(f"Telefono: {contacto['telefono']}")

            labels.append((new_label3, new_label4, new_label5))

        if document.get("favorito"):
            new_label6 = tk.StringVar()
            new_label6.set("Favorito: Si")
            labels.append((new_label6,))
        else:
            new_label6 = tk.StringVar()
            new_label6.set("Favorito: No")
            labels.append((new_label6,))

        separator_label = tk.Label(vo, text="_______________________________")
        labels.append((separator_label,))

    for label_set in labels:
        for label in label_set:
            label_label = tk.Label(vo, textvariable=label)
            label_label.pack()



def delete_set(screen, name, number, t1, t2, t3, t4, t5, t6, btn):
    filtro = {"$or": [{"nombre": name}, {"datos_de_contacto.telefono": number}]}
